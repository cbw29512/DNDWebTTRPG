...
const animated=wishingCakeCombatRules.priest;assert.equal(animated.initiativeModifier,2);assert.equal(animated.shortcuts.find(entry=>entry.id==='ribbon-lash').attackBonus,4);assert.deepEqual(animated.shortcuts.find(entry=>entry.id==='ribbon-lash').damage,[{dice:'1d8+2',type:'slashing'}]);assert.equal(animated.shortcuts.find(entry=>entry.id==='surprise-inside').save.dc,12);assert.equal(animated.shortcuts.find(entry=>entry.id==='surprise-inside').damage[0].dice,'2d6');
const plate=wishingCakeCombatRules.skeleton.shortcuts.find(entry=>entry.id==='bite');assert.deepEqual(plate.damage.map(part=>part.dice),['1d8+3','1d4']);
const pinata=wishingCakeCombatRules['monster-pinata-mimic'];assert.equal(pinata.shortcuts.find(entry=>entry.id==='gore').attackBonus,5);assert.equal(pinata.shortcuts.find(entry=>entry.id==='candy-burst').recharge,'5–6');assert.equal(pinata.shortcuts.find(entry=>entry.id==='candy-burst').save.dc,13);
const sepulchral=wishingCakeCombatRules['monster-sepulchral'];assert.equal(sepulchral.spellSaveDc,13);assert.equal(sepulchral.spellAttackBonus,5);assert.equal(sepulchral.shortcuts.find(entry=>entry.id==='fire-bolt').damage[0].dice,'2d10');assert.equal(wishingCakeCombatRules['hazard-exploding-pinata'].shortcuts.find(entry=>entry.id==='explosion').save.dc,13);assert.equal(wishingCakeCombatRules['hazard-wrapping-machine'].shortcuts.find(entry=>entry.id==='shear').damage[0].dice,'2d6');
...
console.log('Rules-accuracy gate passed: real attack/save/check math, no d20 damage, equipment-derived card fronts, and complete dual-edition pregen rules.');
