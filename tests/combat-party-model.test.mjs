...
combat.turnOrder=['player:s1','group:mimic','player:s2','group:boss'];
combat.activeTurnId='group:mimic';
...
assert.deepEqual(privateSafe.turnOrder,['player:s1','player:s2'],'Hidden monster turns must be removed from public turn order.');
assert.equal(privateSafe.activeTurnId,null,'A hidden monster active turn must not reveal its group identity.');
...
assert.deepEqual(revealed.turnOrder,['player:s1','group:mimic','player:s2']);
assert.equal(revealed.activeTurnId,'group:mimic');
console.log('Party combat setup, grouped monsters, HP scaling, initiative readiness, and reveal-safe public turn state passed.');