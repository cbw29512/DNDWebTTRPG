...
assert.ok(dmView.cards.length>=25);
assert.ok(playerView.cards.length>=8);
assert.equal(dmView.cards.find(card=>card.id==='monster-sepulchral').dmFace.hp,80);
...
assert.equal(playerView.cards.find(card=>card.id==='lantern').face.knownEffect.includes('Once per room'),true);
assert.equal(playerView.actors.find(actor=>actor.id==='sepulchral').hp,undefined);
...
console.log('The Living Table Wishing Cake card pack, readable cards, item uses, grouped initiative, projections, and source registry passed.');
