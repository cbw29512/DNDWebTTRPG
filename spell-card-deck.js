...
if(isPlayer){
 ...
 function deckMarkup(profile){
  ...
  return `<section class="spell-card-deck" aria-labelledby="spell-deck-title"><header class="spell-deck-header"><div><small>PREGEN SPELL MINI-DECK · ${profile.rulesId==='dnd-2024'?'2024 / SRD 5.2.1':'2014 / SRD 5.1'}</small><h3 id="spell-deck-title">Spell Cards</h3><p>Each source carries its own casting ability, slot permission, and free-use rule.</p></div><button type="button" data-spell-long-rest>Long Rest · Restore Spell Uses</button></header><div class="spellcasting-summary"><span>Class ability <strong>${classAbility}</strong></span><span>Source-specific attack/DC <strong>shown on each card</strong></span><div class="spell-slot-tracker">${slotTracker()}</div></div>${missing.length?`<p class="spell-deck-warning">⚠ ${missing.length} spell${missing.length===1?'':'s'} blocked because compact rules data is missing.</p>`:''}${groups}<p class="spell-deck-footnote">A free cast and a spell-slot cast are tracked separately. The ability shown beside each source is the ability used for that source's attack roll or save DC.</p></section>`;
 }
 function render(){
 ...
 function bind(){const deck=document.querySelector('#app .spell-card-deck');if(!deck)return;deck.querySelector('[data-spell-long-rest]')?.addEventListener('click',resetUsage);deck.querySelectorAll('[data-cast-spell]').forEach(button=>button.addEventListener('click',()=>{const name=button.dataset.castSpell;const select=deck.querySelector(`[data-spell-slot-select="${CSS.escape(name)}"]`);const level=Number(select?.value);if(consumeSpellSlot(usage.slots,level)){saveUsage();render();}}));deck.querySelectorAll('[data-free-cast-key]').forEach(button=>button.addEventListener('click',()=>{const use=usage.free[button.dataset.freeCastKey];if(use?.current>0){use.current-=1;saveUsage();render();}}));}
