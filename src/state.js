...
function initialCardVisibility(cards = []) {
  try {
    const revealed = { room:true, priest:true, skeleton:true, hazard:false, treasure:false };
    const cardLabels = { ...CARD_LABELS };
    for (const card of cards) {
      if (!card?.id) continue;
      revealed[card.id] = Boolean(card.revealed);
      cardLabels[card.id] = card.title || card.id;
    }
    return { revealed, cardLabels };
  } catch (error) {
    console.error('[Living Table] Could not initialize card visibility state.', error);
    throw error;
  }
}

export const createInitialState = (cards = []) => {
  const visibility = initialCardVisibility(cards);
  return {
    revision: 0,
    nextEventId: 1,
    roll: "Ready",
    ...
    cardLabels: visibility.cardLabels,
    actors: [
      ["Lyria", "Rogue", 24],
  ...
  try {
    ...
    switch (command.type) {
      ...
      case COMMANDS.TOGGLE_CARD: {
        ...
        event = appendEvent(state, "CARD_VISIBILITY_CHANGED", `DM ${action} ${label}.`, {
          key: command.key,
    ...
    console.error(`[Living Table] Core state command failed for ${command?.type || 'unknown command'}.`, error);
    throw error;
  }
};
