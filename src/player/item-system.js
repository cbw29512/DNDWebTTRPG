...
export const itemCards = Object.freeze([
  ...
  srd({ id:"longbow", name:"Longbow", category:"weapon", rarity:"standard", image:"🏹", validSlots:["mainHand"], attunement:false, edition:"2014/2024", front:"A full-sized martial bow", playerText:"Martial ranged weapon; 1d8 piercing; range 150/600 feet; ammunition, heavy, two-handed. 2024 mastery: Slow.", dmText:"Use Dexterity for attack and damage. Archery Fighting Style adds +2 to the attack roll, not damage. Firing the bow requires two hands, so an active shield is stowed by the equipment doll.", attack:{kind:"ranged",attackAbility:"dexterity",damageDice:"1d8",damageType:"piercing",range:"150/600 ft.",properties:["ammunition","heavy","two-handed"],mastery2024:"Slow"}, effects:[] }),
...
export function equipItem(state,inventory,itemId,slot){
  ...
  const stowed=[];
  if(slot==="mainHand" && item.attack?.properties?.includes("two-handed") && state.equipped.offHand){stowed.push(state.equipped.offHand);state.equipped.offHand=null;}
  if(slot==="offHand"){
    const main=inventory.find(entry=>entry.id===state.equipped.mainHand);
    if(main?.attack?.properties?.includes("two-handed")){stowed.push(state.equipped.mainHand);state.equipped.mainHand=null;}
  }
