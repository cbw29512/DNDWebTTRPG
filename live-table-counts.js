...
function applyCounts(){
 ...
 document.querySelectorAll('#app .card-stack').forEach(stack=>{
   ...
   if(!toggle)return;
   stack.querySelectorAll('.tarot-front .live-stack-quantity').forEach(node=>node.remove());
   let badge=toggle.querySelector(':scope > .live-stack-quantity');
   if(count>1){
     if(!badge){badge=document.createElement('span');badge.className='live-stack-quantity';toggle.append(badge);}
